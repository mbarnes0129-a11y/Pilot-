(() => {
  ...
  // Type helpers
  const TYPES = ["Flame","Aqua","Nature","Storm","Light","Shadow"];
  ...
  try {
    const saved = localStorage.getItem("digicoach_fix_all");
  ...
  if (state.lastDayKey !== todayKey) {
    ...
    resetNoteEl.textContent = "Daily quests track for "+ todayKey +".";
  }
  if (state.lastCampaignKey !== todayKey) {
    ...
    if (!localStorage.getItem("digicoach_fix_all")) state.campaignDay = 1;
  }

  const thresholds = [1,5,12,20,30,40,55];
  const stageNames = ["Egg","In Training","Rookie","Champion","Ultimate","Mega","Grand Mega"];
  function save(){ localStorage.setItem("digicoach_fix_all", JSON.stringify(state)); }
  ...
  if (!btnStartBattle || !moveA || !storeItems) {
  ...
  // Cache bypass button — unregister SW and reload
  $("#btnBypassCache").addEventListener("click", async () => {
    if ('serviceWorker' in navigator) {
      const regs = await navigator.serviceWorker.getRegistrations();
      for (const r of regs) await r.unregister();
    }
    localStorage.removeItem("digicoach_fix_all"); // optional: comment to keep save
  ...
  // Stage image uploads — guaranteed user gesture, single click -> opens correct input
  $$(".avatar-actions button").forEach(btn => {
    btn.addEventListener("click", () => {
      const stage = btn.getAttribute("data-pick");
      const input = $("#imgStage" + stage);
      if (!input) return;
      input.value = ""; // reset so same file can re-upload
      input.click(); // iOS-friendly because in user gesture
    });
  });
  for (let i=1;i<=7;i++){
    const el = $("#imgStage"+i);
    el.addEventListener("change", () => {
      ...
      const rd = new FileReader(); rd.onload = () => {
        state.stageImages[String(i)] = rd.result;
        save(); refreshAvatar();
        alert("Stage "+i+" image saved.");
      };
      rd.readAsDataURL(f);
    });
  }
  ...
  function refreshAvatar(){
    ...
    petImg.src = img;
    stageName.textContent = stageLabel(state.stage);
    typeChip.textContent = state.companionType;
    setBuddyHPUI();
  }
  ...
  // Mini helpers
  function setBuddyHPUI(){
    buddyHPBar.style.width = Math.max(0, Math.round((state.playerHP/state.playerHPMax)*100)) + "%";
    buddyHPText.textContent = `${state.playerHP}/${state.playerHPMax}`;
  }
  function refreshBattleSummary(){
    campDayEl.textContent = state.campaignDay;
    battleNoEl.textContent = Math.min(3, state.battlesToday+1);
    const type = todaysBattleType();
    battleTypeEl.textContent = (type==="boss"?"Boss":"Daily");
    extraPoolEl.textContent = Math.min(4, tasksDone() + (parseInt(state.manualBonus)||0));
    battleStatusEl.textContent = (state.battlesToday>=3 || type==="done") ? "All battles done" : "Ready";
    btnStartBattle.disabled = (state.battlesToday>=3 || type==="done");
    btnResumeBattle.classList.toggle("hidden", !battleState.active);
  }
  if (manualBonusEl) manualBonusEl.addEventListener("change", () => {
  ...
  function startBattle(){
    ...
    battleState.enemyLabel = (battleState.isBoss ? "Boss " : "") + MOB_NAMES[battleState.enemyStageNum][rng(0,2)];
    ...
    const btns = [moveA,moveB,moveC,moveD];
    btns.forEach((btn,i)=>{ btn.textContent = battleState.youMoves[i].name; btn.disabled=false; });
    refreshItemSelect();
  ...
  function yourAction(kindIndex){
    ...
    // extra actions chain
    if (battleState.extraPool>0){ battleState.extraPool -= 1; $("#apLeft").textContent = battleState.extraPool; tickTurns(battleState.you); return; }
    // enemy turn
    tickTurns(battleState.you);
    const em = battleState.enemyMoves[rng(0,battleState.enemyMoves.length-1)];
  ...
  function refreshItemSelect(){
    ...
    Object.entries(state.inv).forEach(([k,v])=>{ if (v>0) parts.append?parts.append():null; });
    const opts = [];
  ...
  $("#useItem").addEventListener("click", () => {
    ...
    if (key==="elixir"){ battleState.extraPool = Math.min(4, battleState.extraPool + 1); $("#apLeft").textContent = battleState.extraPool; battleLog.textContent += `\nYou quaff an Elixir. +1 extra action.`; }
  ...
  function endBattle(victory){
    [moveA,moveB,moveC,moveD].forEach(b=>b.disabled=true);
    battleState.active=false;
    let rewardXP=0, rewardCoins=0;
    if (victory){
      rewardXP = battleState.isBoss ? 80 : 25; rewardCoins = battleState.isBoss ? 20 : 5;
      state.xp += rewardXP; state.coins += rewardCoins; battleLog.textContent += `\nVictory! +${rewardXP} XP, +${rewardCoins} coins.`;
    } else {
      const newLevel = Math.max(1, state.level - 3);
      state.xp = (newLevel-1)*100;
      state.playerHP = Math.max(1, Math.round(state.playerHPMax*0.3));
      battleLog.textContent += `\nDefeat... You fall back 3 levels. HP set to 30%.`;
    }
    state.playerHP = Math.max(1, battleState.you.hp); setBuddyHPUI();
    state.battlesToday = Math.min(3, state.battlesToday + 1);
    save(); refreshStats(); refreshBattleSummary();
  }

  // Move button hooks (ensure single-attach)
  [ ["moveA",0],["moveB",1],["moveC",2],["moveD",3] ].forEach(([id,idx])=>{
    const btn = document.getElementById(id);
    btn.addEventListener("click", () => yourAction(idx), { once:false });
  });
  $("#btnBattleClose").addEventListener("click", () => safeCloseDialog(battleDialog));
  btnStartBattle.addEventListener("click", startBattle);
  btnResumeBattle.addEventListener("click", () => safeShowDialog(battleDialog));

  // Utility
  function escapeHtml(s){ return s.replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])) }

  function boot(){
    refreshAvatar(); refreshStats(); renderTasks(); refreshBattleSummary(); refreshItemSelect();
    // Ensure app.js cache bust matches SW param
    const selfScript = document.querySelector('script[src^="app.js"]');
    if (selfScript && window.__DIGI_VERSION__) selfScript.src = `app.js?v=${window.__DIGI_VERSION__}`;
  }
  // DOMContentLoaded not strictly needed since script at end, but safe:
